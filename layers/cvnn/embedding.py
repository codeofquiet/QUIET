...
def phase_embedding_layer(input_shape, input_dim, embedding_dim = 1,trainable = True,l2_reg=0.0000005):
    ...
    ###input_length=input_shape 输入序列的长度 12 
    ###input_dim词汇表的大小 2951 
    ###embedding_dim词嵌入的长度 50
    ###embeddings_initializer=RandomUniform(minval=0, maxval=2*math.pi) 初始化为0-2pai的随机数
    ###并设置为可训练的
    print("qnn/layers/cvnn/embedding.py input_dim embedding_dim input_length")
    print(input_dim)
    print(embedding_dim)
    print(input_shape)
...
def amplitude_embedding_layer(embedding_matrix, input_shape, trainable = False, random_init = True,l2_reg=0.0000005):
    ...
    print("if ramdon init")
    print(random_init)
    print(np.transpose(embedding_matrix))
    print('\n\n')
    if(random_init):
