...
def getmustardweight(batch_size):
        weight = [1]
        allWeights = []
...
class TrainDataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, datatype, batch_size, shuffle=True):
        ...
        if self.datatype == 'train':
            datafile = '../../dataset/mustard/mustard-dataset-train.csv'
            self.datalen = 500
        elif self.datatype == 'dev':
            datafile = '../../dataset/mustard/mustard-dataset-dev.csv'
            self.datalen = 90
        elif self.datatype == 'test':
            datafile = '../../dataset/mustard/mustard-dataset-test.csv'
            self.datalen = 100
    ...
    def __getitem__(self, index):
        ...
        for i in batch_indexs:
            ...
            textPath = \
            "../../dataset/mustard/textFeature/"+self.datatype+"/"+uttName+"/"
            textFea = [np.loadtxt(
                            textPath+str(num),
                            dtype=float,
            ...
            imagePath = \
                "../../dataset/mustard/imageFeature/"+self.datatype+"/"+uttName+"/"
            imageFea = [np.loadtxt(
                            imagePath+str(num),
                            dtype=float,
                            delimiter=",") for num in range(4)]
            ...
            audioPath = "../../dataset/mustard/audioFeature/"+self.datatype+"/"
        ...
        weight_out = getmustardweight(len(batch_indexs))
...
def getValidData(datatype):
    if datatype == 'dev':
        audioPath = "../../dataset/mustard/audioFeature/dev/"
        textPath = "../../dataset/mustard/textFeature/dev/"
        imagePath = "../../dataset/mustard/imageFeature/dev/"
        vallen = 90
        datafile = '../../dataset/mustard/mustard-dataset-dev.csv'
        uttDict, uttNameList = readcsv(datafile)
        uttDict = process_utt_dict(uttDict)
    else:
        audioPath = "../../dataset/mustard/audioFeature/test/"
        textPath = "../../dataset/mustard/textFeature/test/"
        imagePath = "../../dataset/mustard/imageFeature/test/"
        vallen = 100
        datafile = '../../dataset/mustard/mustard-dataset-test.csv'
        uttDict, uttNameList = readcsv(datafile)
        uttDict = process_utt_dict(uttDict)
